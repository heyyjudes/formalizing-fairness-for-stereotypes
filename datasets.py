...
class Dataset:
    def __init__(self, dataset_name: str, **kwargs) -> None:
        ...
        self.acs_dict = {
            "income": (ACSIncome, acs.ACSIncome_categories),
    ...
    def get_cov_yg(self, sensitive_ind=0):
        ...
        for i in range(self.x_train.shape[1]):
            ...
            if np.std(self.x_train[:, i]) > 0:
                x = (self.x_train[:, i] - np.mean(self.x_train[:, i])) / np.std(
                    self.x_train[:, i]
                )
    ...
    def preprocess_biasbios(
        self, target_occ: str = "professor", rep: str = "WE", balanced=False
    ):
        """
        ...
        :return:
        """
        with open(DATA_DIR / "bios/BIOS.pkl", "rb") as f:
            x = pickle.load(f)
        ...
        if rep == "WE":
            ...
            x_occ = balanced_bios.apply(
                lambda row: row["bio"][row["start_pos"] :], axis=1
            ).tolist()
            self.x_raw = x_occ
            ...
            self.vectorizer = CountVectorizer(
                analyzer="word", stop_words="english", max_features=5000
            )
    ...
    def preprocess_nels(self, normalize: bool = True, condensed_features: bool = False):
        ...
        print(
            f'Limiting entries to two racial groups 3 n={len(df[df["RACE"] == 3])} and 4 n={len(df[df["RACE"] == 4])}'
    ...
    def preprocess_bank(self, remove_sensitive: bool = False):
        ...
        sensitive_attr_keys = [
            key
            for key in df_encoded.keys()
            if (key.startswith("age_group") or key.startswith("contact"))
        ]
    ...
    def preprocess_lawschool(self, remove_sensitive: bool = False):
        ...
        sensitive_attr_keys = [
            key
            for key in df_encoded.keys()
            if key.startswith("race1") or key.startswith("gender")
        ]
        s = df_encoded[sensitive_attr_keys]
        x = df_encoded.drop(sensitive_attr_keys, axis=1)
    ...
    def preprocess_acs(
        self, dataset_name: str, remove_sensitive: bool = False, state="CA"
    ):
        self.exclude_attr_list = [
        ...
        sensitive_attr_keys = [
            key for key in x.keys() if key.startswith("RAC1P") or key.startswith("SEX")
        ]
    ...
    def split_train_test(self, test_size=0.3, random_state=0):
        ...
        (
            ...
            self.x, self.y, self.g, test_size=test_size, random_state=random_state
        )
        ...
        if self.dataset_name == "biasbios":
            ...
            self.y_train = np.where(
                self.y_train == self.class_map[self.tgt_class], 1, 0
            )
